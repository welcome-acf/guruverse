  ...
  <div class="page active" id="page-dashboard">
    ...
    <div class="hero-section mb-24">
      ...
      <div class="hero-illustration">
        ...
        <div class="hero-img-3d-wrap">
          ...
          <img class="hero-main-img" src="{{ asset('asset/img/hero-teachers.png') }}" alt="Guru-guru GuruVerse" loading="eager">
        </div>
      </div>
    </div>
    ...
    <div class="layout-two-col mb-24">
      <div>
        ...
        <!-- $latest_enrollment sudah di-passing secara otomatis dari Controller menggunakan Eloquent -->
        ...
        @php else: 
          $pct = $latest_enrollment['total_modules'] > 0 ? round(($latest_enrollment['completed_modules'] / $latest_enrollment['total_modules']) * 100) : 0; @endphp
      ...
      <div class="card card-body">
        ...
        <div class="cal-days">
          ...
          @php // Simple dynamic calendar generator for current month
            $y = date('Y'); $m = date('n');
    ...
    <!-- $rekomendasi dan $rekomendasi_enrolled sudah di-passing secara otomatis dari Controller menggunakan Eloquent -->
    ...
    @else
      @php // Cek apakah semua kelas yang ditampilkan sudah selesai
        $all_completed = $rekomendasi_enrolled && count(array_filter($rekomendasi, fn($r) => $r['enroll_status'] === 'completed')) === count($rekomendasi); @endphp
      @if ($rekomendasi_enrolled && !$all_completed)
      ...
      <div class="kelas-grid mb-24">
        @php foreach ($rekomendasi as $rk): 
          ...
          $is_rk_completed = ($rk['enroll_status'] ?? '') === 'completed'; @endphp
        <div class="card p-0 overflow-hidden" style="display:flex;flex-direction:column;cursor:pointer" onclick="{{ $is_rk_completed ? (!empty($rk['pdf_path']) ? "viewCertificate('".htmlspecialchars($rk['pdf_path'])."')" : "") : "openCoursePlayer({$rk['id']})" }}">
          <div style="height:140px;background:{{ $is_rk_completed ? 'linear-gradient(135deg,#10b981,#059669)' : 'linear-gradient(135deg,var(--c-primary-light),var(--c-primary))' }};position:relative; {{ $rk['thumbnail'] ? 'background-image:url(\''.asset('uploads/thumbnails/'.$rk['thumbnail']).'\');background-size:cover;background-position:center;' : '' }}">
            ...
            @if ($rekomendasi_enrolled && !empty($rk['enroll_status']))
            <div style="position:absolute;bottom:10px;left:10px;background:rgba(255,255,255,0.92);color:#2d3436;font-size:10px;font-weight:800;padding:3px 8px;border-radius:6px;">
    ...
    <div style="background: rgba(108, 92, 231, 0.04); border-radius: 24px; padding: 32px 24px; margin-bottom: 32px; border: 1px solid rgba(108, 92, 231, 0.08);">
      ...
      <!-- $populer sudah di-passing secara otomatis dari Controller menggunakan Eloquent -->
      ...
      <div class="kelas-grid">
        ...
        <div class="card p-0 overflow-hidden" style="display:flex;flex-direction:column;cursor:pointer;" onclick="showPage('detail_kelas')">
          <div style="height:140px;background:linear-gradient(135deg,var(--c-primary-light),var(--c-primary));position:relative; {{ $pop['thumbnail'] ? 'background-image:url(\''.asset('uploads/thumbnails/'.$pop['thumbnail']).'\');background-size:cover;background-position:center;' : '' }}">
            <div style="position:absolute;top:12px;right:12px;background:rgba(255,255,255,0.2);backdrop-filter:blur(4px);color:#fff;font-size:10px;font-weight:800;padding:4px 10px;border-radius:20px;">
              {{ htmlspecialchars($pop['category']) }}
            </div>
            <div style="position:absolute;bottom:12px;left:12px;background:rgba(255,255,255,0.92);color:#2d3436;font-size:10px;font-weight:800;padding:4px 8px;border-radius:8px;display:flex;align-items:center;gap:4px">
    ...
    <div class="community-banner mb-24">
      ...
      <div style="flex-shrink:0"><img src="{{ asset('asset/img/community_teachers_muslim (2).png') }}" alt="Komunitas Guru" loading="lazy" style="width:280px;height:190px;object-fit:cover;border-radius:16px;display:block;box-shadow:0 4px 20px rgba(0,0,0,0.3)"></div>
    </div>
    ...
    <div class="features-grid mb-24">
      <div class="feature-card">
        <div class="feature-illustration" style="padding:0;overflow:hidden;border-radius:10px"><img src="{{ asset('asset/img/hero_classroom_hub.png') }}" alt="Kelas Online" loading="lazy" style="width:100%;height:100%;object-fit:cover;display:block"></div>
      ...
      <div class="feature-card">
        <div class="feature-illustration" style="padding:0;overflow:hidden;border-radius:10px"><img src="{{ asset('asset/img/pilar_learning.png') }}" alt="Modul Belajar" loading="lazy" style="width:100%;height:100%;object-fit:cover;display:block"></div>
      ...
      <div class="feature-card">
        <div class="feature-illustration" style="padding:0;overflow:hidden;border-radius:10px"><img src="{{ asset('asset/img/premium-teacher-certificate.png') }}" alt="Sertifikat" loading="lazy" style="width:100%;height:100%;object-fit:cover;object-position:top center;display:block"></div>
