...
.diskusi-tab.active { background: #fff; color: var(--c-primary); box-shadow: 0 1px 4px rgba(0,0,0,0.1); }
.diskusi-search-mini {
  display: flex; align-items: center; gap: 8px;
  background: #fff; border: 1.5px solid var(--c-border); border-radius: 10px;
...
.dsk-card { background: #fff; border: 1px solid var(--c-border); border-radius: 16px; padding: 20px; }
...
.dsk-empty {
  ...
  background: #fff; border: 2px dashed var(--c-border); border-radius: 16px;
}
.dsk-empty-icon {
  ...
  <div>
    ...
    <div class="diskusi-filter-bar">
      <div class="diskusi-tabs">
        <button class="diskusi-tab active" data-tab="semua">Semua</button>
        <button class="diskusi-tab" data-tab="terbaru">Terbaru</button>
        <button class="diskusi-tab" data-tab="populer">Populer</button>
      </div>
...
function submitDiskusi(e) {
  ...
  fetch('pages/diskusi_submit.php', {
    method: 'POST',
...
function deleteTopic(id) {
  ...
  fetch('pages/diskusi_delete.php', {
    method: 'POST',
    headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
    body: 'id=' + id
  })
