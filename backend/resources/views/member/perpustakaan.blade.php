...
@section('title', ucwords(str_replace('_', ' ', 'perpustakaan')))

@section('content')

<!-- Data ebooks dikirim via MemberLibraryController -->
<style>
  .gv-cart-float {
    position: fixed;
    bottom: 96px;
...
<div class="page active" id="page-perpustakaan">
  ...
  <script>
    ...
    function ebReadOnline() {
      ...
      if (isOfficeFile) {
          ...
          window.open('{{ url('/guru-belajar/member/read_book.php') }}?url=' + encodeURIComponent(_ebModalPdfUrl) + '&title=' + encodeURIComponent(_ebModalTitle), '_blank');
      }
    }
