...
<div class="page active" id="page-kirim-karya">
  ...
  <div style="position:relative; overflow:hidden; border-radius:20px; margin-bottom:28px;
    ...
    <div style="position:relative;z-index:1;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;">
      <div>
        ...
        <h2 style="margin:0 0 6px;font-size:1.5rem;font-weight:900;line-height:1.2;color:#fff;">Kirim Karya Inovatif</h2>
  ...
  <div style="margin-bottom:28px;">
    ...
    @if (empty($karyas))
    <div style="text-align:center;padding:48px 20px;border:2px dashed var(--c-border);border-radius:16px;color:var(--c-text-muted);">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:14px;">
      ...
      @foreach ($top3 as $i => $k)
        @php
        $m       = $medals[$i];
        $is_mine = ((int)$k['member_id'] === (int)$uid);
    ...
    <div id="all-karya-list" style="display:none; margin-top:14px; display:flex; flex-direction:column; gap:10px;">
      @foreach (array_slice($karyas, 3) as $idx => $k)
        @php
        $ri = $idx + 3;
        $voted   = (bool)$k['sudah_vote'];
        $is_mine = ((int)$k['member_id'] === (int)$uid);
            ...
            <div>
              ...
              <div style="position:relative;" id="custom-select-wrapper">
                ...
                <div id="custom-select-options"
                  ...
                  <div class="c-opt" onclick="selectOption('Media Pembelajaran Interaktif', 'Media Pembelajaran Interaktif')">Media Pembelajaran Interaktif</div>
                  <div class="c-opt" onclick="selectOption('Artikel / Jurnal Praktik Baik', 'Artikel / Jurnal Praktik Baik')">Artikel / Jurnal Praktik Baik</div>
                ...
                <select id="karya-jenis" required style="display:none;">
                  ...
                  <option value="Media Pembelajaran Interaktif">Media Pembelajaran Interaktif</option>
                  <option value="Artikel / Jurnal Praktik Baik">Artikel / Jurnal Praktik Baik</option>
...
async function submitKarya(e) {
    ...
    try {
        const res  = await fetch("{{ route('member.mengajar.gamifikasi.karya.submit') }}", { 
            method:'POST', 
            body:fd,
            headers: {
