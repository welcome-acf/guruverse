...
@section('title', ucwords(str_replace('_', ' ', 'diskusi_detail')))

@section('content')

<div class="page active" id="page-diskusi_detail">
<style>
...
function openDiskusiDetail(topicId) {
    ...
    // Fetch data
    fetch('/diskusi/api/' + topicId)
        .then(res => res.json())
        .then(data => {
            if (data.success) {
                renderDiskusiDetail(data.data, data.data.replies);
            } else {
...
function renderDiskusiDetail(topic, replies) {
    ...
    let html = '';
    replies.forEach(r => {
        const author = r.author_name || 'Member';
        let avatarHtml = '';
        if (r.photo_path) {
