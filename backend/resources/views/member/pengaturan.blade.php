...
@section('title', ucwords(str_replace('_', ' ', 'pengaturan')))
  ...
  <div style="display:grid;grid-template-columns:1fr 300px;gap:20px;align-items:start">
    ...
    <div class="card card-body-lg">
      ...
      <div class="flex items-center gap-16 mb-24">
        <div class="avatar-upload-wrap" onclick="document.getElementById('avatarFileInput').click()">
          <div class="avatar avatar-xl" style="width:64px;height:64px;background:linear-gradient(135deg,var(--c-primary),var(--c-primary-light));color:#fff;font-size:22px;font-weight:800" id="avatarPreview">
        ...
        <input type="file" id="avatarFileInput" accept="image/*" style="display:none" onchange="previewAndUploadAvatar(this)">
        <div>
          ...
          <div style="display:flex;gap:8px">
            <button class="btn btn-outline btn-sm" type="button" onclick="document.getElementById('avatarFileInput').click()">Ganti Foto</button>
            <button class="btn btn-ghost btn-sm" type="button" onclick="deleteAvatar()">Hapus Foto</button>
          </div>
        </div>
      </div>
      ...
      <form id="form-profile" onsubmit="saveProfile(event)">
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px">
          ...
          <div class="form-group">
            ...
            <select name="institution" class="form-input">
              @foreach (['Guru SD','Guru SMP','Guru SMA/SMK','Dosen','Lainnya'] as $j)
        ...
        <div class="flex gap-12">
          ...
          <button type="button" class="btn btn-ghost" onclick="window.location.href='{{ route('member.dashboard') }}'">Batal</button>
        </div>
      </form>
    </div>
...
<div class="sett-panel" id="sett-notifikasi">
  <div style="max-width:600px">
    <div class="card card-body-lg">
      ...
      <div class="divider"></div>
      <button class="btn btn-primary btn-sm" onclick="saveNotifPrefs(this)">Simpan Preferensi</button>
    </div>
  </div>
</div>
...
<div class="sett-panel" id="sett-keamanan">
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:20px;max-width:800px">
    ...
    <div class="card card-body-lg">
      ...
      <button class="sett-security-btn" onclick="showChangePasswordModal()">
...
function previewAndUploadAvatar(input) {
  ...
  if (file.size > maxSize) {
    gbShowAlert('Gagal', 'Ukuran foto maksimal 2MB.', 'error');
    return;
  }
  ...
  fetch('api/update_profile.php?action=avatar', { method: 'POST', body: fd })
    .then(r => r.json())
    .then(d => {
      if (d.success) {
        gbShowAlert('Berhasil', 'Foto profil berhasil diperbarui!', 'success');
      } else {
        gbShowAlert('Gagal', d.message || 'Gagal mengunggah foto.', 'error');
      }
    })
    .catch(() => gbShowAlert('Error', 'Terjadi kesalahan koneksi.', 'error'));
}

function deleteAvatar() {
  gbShowConfirm('Hapus Foto?', 'Yakin ingin menghapus foto profil Anda?', function() {
    fetch('api/update_profile.php?action=avatar_delete', { method: 'POST' })
      .then(r => r.json())
      .then(d => {
        if (d.success) {
          const preview = document.getElementById('avatarPreview');
          if (preview) preview.innerHTML = '{{ htmlspecialchars($user_initials) }}';
          gbShowAlert('Berhasil', 'Foto profil dihapus.', 'success');
        }
      });
  });
}
...
function showChangePasswordModal() {
  const content = `
    <div style="text-align:left">
      <div class="form-group" style="margin-bottom:14px">
        <label class="form-label" style="font-size:12px;font-weight:700;margin-bottom:6px;display:block">Password Lama</label>
        <input type="password" id="cpOldPass" class="form-input" placeholder="Password saat ini" style="width:100%;padding:10px;border:1px solid var(--c-border);border-radius:8px;font-size:13px;background:transparent;color:var(--c-text)">
      </div>
      <div class="form-group" style="margin-bottom:14px">
        <label class="form-label" style="font-size:12px;font-weight:700;margin-bottom:6px;display:block">Password Baru</label>
        <input type="password" id="cpNewPass" class="form-input" placeholder="Minimal 8 karakter" style="width:100%;padding:10px;border:1px solid var(--c-border);border-radius:8px;font-size:13px;background:transparent;color:var(--c-text)">
      </div>
      <div class="form-group">
        <label class="form-label" style="font-size:12px;font-weight:700;margin-bottom:6px;display:block">Konfirmasi Password Baru</label>
        <input type="password" id="cpConfirmPass" class="form-input" placeholder="Ulangi password baru" style="width:100%;padding:10px;border:1px solid var(--c-border);border-radius:8px;font-size:13px;background:transparent;color:var(--c-text)">
      </div>
    </div>`;

  document.getElementById('gbCustomModal').style.display = 'flex';
  document.getElementById('gbModalContainer').style.maxWidth = '420px';
  document.getElementById('gbModalTitle').textContent = '🔒 Ubah Password';
  document.getElementById('gbModalBody').innerHTML = content;
  document.getElementById('gbModalCancel').style.display = 'inline-flex';

  const okBtn = document.getElementById('gbModalOk');
  okBtn.textContent = 'Simpan Password';
  ...
  okBtn.onclick = function() {
    ...
    if (!oldPass || !newPass || !confirmPass) {
      gbShowAlert('Peringatan', 'Semua kolom wajib diisi.', 'info');
      return;
    }
    if (newPass.length < 8) {
      gbShowAlert('Peringatan', 'Password baru minimal 8 karakter.', 'info');
      return;
    }
    if (newPass !== confirmPass) {
      gbShowAlert('Peringatan', 'Konfirmasi password tidak cocok.', 'error');
      return;
    }
    ...
    fetch('api/update_profile.php?action=password', { method: 'POST', body: fd })
      .then(r => r.json())
      .then(d => {
        if (d.success) {
          gbShowAlert('Berhasil! ✅', 'Password berhasil diperbarui.', 'success');
        } else {
          gbShowAlert('Gagal', d.message || 'Gagal mengubah password.', 'error');
        }
      })
      .catch(() => gbShowAlert('Error', 'Terjadi kesalahan koneksi.', 'error'));
  };
...
function saveNotifPrefs(btn) {
  const toggles = document.querySelectorAll('#sett-notifikasi .toggle');
  const prefs = {};
  toggles.forEach(function(t, i) {
    prefs['notif_' + i] = t.classList.contains('on') ? 1 : 0;
  });
  ...
  const fd = new FormData();
  fd.append('notif_prefs', JSON.stringify(prefs));
  fetch('api/update_profile.php?action=notif', { method: 'POST', body: fd })
    .then(r => r.json())
    .then(d => {
      gbShowAlert('Berhasil', 'Preferensi notifikasi disimpan!', 'success');
    })
    .catch(() => gbShowAlert('Berhasil', 'Preferensi disimpan secara lokal.', 'success'))
    .finally(() => {
      btn.disabled = false;
      btn.textContent = 'Simpan Preferensi';
    });
}
</script>
