...
<title>@yield('title', 'Guru Belajar') &mdash; Guruverse.ID</title>
...
<div class="layout">
  ...
  <aside class="sidebar">
    ...
    <nav class="sidebar-nav">
      ...
      <a class="nav-item {{ Route::is('member.dashboard') ? 'active' : '' }}" href="{{ route('member.dashboard') }}">
      ...
      <a class="nav-item {{ Route::is('member.kelas') ? 'active' : '' }}" href="{{ route('member.kelas') }}">
        <span class="nav-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 10v6M2 10l10-5 10 5-10 5z"/><path d="M6 12v5c3 3 9 3 12 0v-5"/></svg></span> <span class="nav-text">Kelas Saya</span>
      </a>
      <a class="nav-item {{ Route::is('member.sertifikat') ? 'active' : '' }}" href="{{ route('member.sertifikat') }}">
        <span class="nav-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="8" r="6"/><path d="M15.477 12.89L17 22l-5-3-5 3 1.523-9.11"/></svg></span> <span class="nav-text">Sertifikat</span>
      </a>
      ...
      <a class="nav-item {{ Route::is('member.diskusi') ? 'active' : '' }}" href="{{ route('member.diskusi') }}">
        <span class="nav-icon"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg></span> <span class="nav-text">Diskusi</span>
      </a>
      <span class="nav-label">Akun</span>
      <a class="nav-item {{ Route::is('member.pengaturan') ? 'active' : '' }}" href="{{ route('member.pengaturan') }}">
...
function showPage(page) {
    const map = {
        'modul': '{{ route("member.modul") }}',
        'cart': '{{ url("member/cart") }}', // fallback
        'detail_kelas': '{{ route("member.kelas") }}'
    };
    if (map[page]) window.location.href = map[page];
}

function openCoursePlayer(courseId) {
    window.location.href = '{{ route("member.modul") }}?course_id=' + courseId;
}

function gvEnroll(courseId) {
    // For now redirect to module page
    window.location.href = '{{ route("member.modul") }}?course_id=' + courseId;
}
